...
+ (UIImage *)imageByCompress:(UIImage *)oriImage withHeightLimit:(CGFloat)limitHeight andWidthLimit:(CGFloat)limitWidth {
    ...
    if (oriSize.width > limitWidth && oriSize.width > oriSize.height) {
        newSize.width = limitWidth;
        newSize.height = oriSize.height * (limitWidth / oriSize.width);
    }
    else if (oriSize.height > limitHeight && oriSize.height > oriSize.width) {
        newSize.height = limitHeight;
        newSize.width = oriSize.width * (limitHeight / oriSize.height);
    }

    UIGraphicsBeginImageContext(newSize);
    [oriImage drawInRect:(CGRect){0,0,newSize.width,newSize.height}];
